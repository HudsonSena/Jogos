...
// -----------------------------------------------------------------------------
// INICIALIZAÇÃO, GERAÇÃO E ATUALIZAÇÃO
// -----------------------------------------------------------------------------
...
void gerar_item(Item itens[], TipoItem tipo) {
    for (int i = 0; i < MAX_ITENS; i++) {
        if (!itens[i].ativo) {
            ...
            // Posição X: Aleatória na área segura
            int margem = (int)(LARGURA_TELA * 0.10); // 10% de margem
...
void atualizar_itens(Item itens[]) {
    for (int i = 0; i < MAX_ITENS; i++) {
        if (itens[i].ativo) {
            ...
            // Se o item cair no chão, ele desaparece
...
void desenhar_itens(Item itens[]) {
    for (int i = 0; i < MAX_ITENS; i++) {
        if (itens[i].ativo) {
            ...
            switch (itens[i].tipo) {
                case ITEM_VIDA:
                    cor = al_map_rgb(0, 255, 0); // Verde para Vida
                    break;
                case ITEM_BONUS_1:
            ...
            al_draw_filled_rectangle(itens[i].x, itens[i].y,
                                     itens[i].x + ITEM_LARGURA, itens[i].y + ITEM_ALTURA,
                                     cor);
        }
    }
}
...
void checar_coleta_item(Dinossauro *dino, Item itens[]) {
    // Coordenadas do Dinossauro (Corpo Inteiro - Para simplificar a coleta)
    float dx1 = dino->x;
    float dy1 = dino->y;
    ...
    for (int i = 0; i < MAX_ITENS; i++) {
        if (itens[i].ativo) {
            ...
            if (dx1 < ix2 && dx2 > ix1 && dy1 < iy2 && dy2 > iy1) {
                ...
                itens[i].ativo = 0; // Desativa o item após a coleta
            }
        }
    }
}